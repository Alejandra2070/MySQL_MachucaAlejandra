CREATE DATABASE JardineriaAlejandra;

USE JardineriaAlejandra;

-- drop database JardineriaAlejandra;
CREATE TABLE gama_producto(
	gama VARCHAR(50) PRIMARY KEY,
    description_texto TEXT,
...
-- alter table cliente modify column codigo_cliente INT;
-- ALTER TABLE cliente MODIFY linea_direccion2 VARCHAR(50) NULL;
-- ALTER TABLE cliente MODIFY region VARCHAR(50) NULL;
...
ALTER TABLE pedido MODIFY fecha_pedido DATE;
ALTER TABLE pedido MODIFY fecha_esperada DATE;
ALTER TABLE pedido MODIFY fecha_entrega DATE;
ALTER TABLE pedido MODIFY estado VARCHAR(15);
ALTER TABLE pedido MODIFY comentarios TEXT;
...
-- drop table pago1;
-- ALTER TABLE pago MODIFY codigo_cliente INT(11) NOT NULL;
-- ALTER TABLE pago MODIFY forma_pago VARCHAR(40);
-- ALTER TABLE pago MODIFY id_transaccion VARCHAR(50);
-- ALTER TABLE pago MODIFY fecha_pago DATE;
-- ALTER TABLE pago MODIFY total DECIMAL(15,2);
...
SELECT ciudad,telefono FROM oficina WHERE ciudad IN ('Madrid','Barcelona'); -- 2. Devuelve un listado con la ciudad y el teléfono de las oficinas de España.
...
SELECT puesto,nombre,apellido1,apellido2,email FROM empleado WHERE codigo_jefe = 1;-- 4. Devuelve el nombre del puesto, nombre, apellidos y email del jefe de la empresa. -- Arreglaaar!!
...
SELECT nombre_cliente FROM cliente WHERE pais = 'Spain'; -- 6. Devuelve un listado con el nombre de los todos los clientes españoles.
...
SELECT DISTINCT codigo_cliente FROM pedido WHERE YEAR(fecha_pedido) = 2008 AND estado = 'Pagado'; --  8. Utilizando la función YEAR de MySQL. -- Arreglaaar!!
SELECT codigo_pedido,codigo_cliente,fecha_esperada,fecha_entrega FROM pedido WHERE fecha_entrega > fecha_esperada OR fecha_entrega IS NULL; -- 9. Devuelve un listado con el código de pedido, código de cliente, fecha esperada y fecha de entrega de los pedidos que no han sido entregados a tiempo.
-- Devuelve un listado con el código de pedido, código de cliente, fecha esperada y fecha de entrega de los pedidos cuya fecha de entrega ha sido al menos dos días antes de la fecha esperada.
SELECT codigo_pedido,codigo_cliente,fecha_esperada,fecha_entrega FROM pedido WHERE fecha_entrega < ADDDATE(fecha_esperada, -2); -- 10. Utilizando la función ADDDATE de MySQL.
SELECT * FROM pedido WHERE YEAR(fecha_pedido) = 2009; -- 11. Devuelve un listado de todos los pedidos que fueron en 2009.
SELECT * FROM pedido WHERE MONTH(fecha_pedido) = 1; -- 12. Devuelve un listado de todos los pedidos que han sido  en el mes de enero de cualquier año.
SELECT * FROM pago WHERE YEAR(fecha_pago) = 2008 AND forma_pago = 'PayPal' ORDER BY total DESC; -- 13. Devuelve un listado con todos los pagos que se realizaron en el año 2008 mediante Paypal. Ordene el resultado de mayor a menor.
...
SELECT * FROM producto WHERE gama = 'Ornamentales' AND cantidad_en_stock > 100 ORDER BY precio_venta DESC; -- 15. Devuelve un listado con todos los productos que pertenecen a la gama Ornamentales y que tienen más de 100 unidades en stock. El listado deberá estar ordenado por su precio de venta, mostrando en primer lugar los de mayor precio.
SELECT nombre_cliente, codigo_cliente, codigo_empleado_rep_ventas FROM cliente WHERE ciudad = 'Madrid' AND codigo_empleado_rep_ventas in (11,30); -- 16. Devuelve un listado con todos los clientes que sean de la ciudad de Madrid y cuyo representante de ventas tenga el código de empleado 11 o 30.

-- Consultas multitabla (Composición interna) (OPCIONAL)

SELECT nombre_cliente, nombre_contacto, apellido_contacto FROM cliente c INNER JOIN empleado e ON c.codigo_empleado_rep_ventas=e.codigo_empleado; -- Obtén un listado con el nombre de cada cliente y el nombre y apellido de su representante de ventas.
SELECT nombre_cliente, nombre_contacto FROM pago p INNER JOIN cliente c ON p.codigo_cliente=c.codigo_cliente INNER JOIN empleado e ON c.codigo_empleado_rep_ventas=e.codigo_empleado; -- 2. Muestra el nombre de los clientes que hayan realizado pagos junto con el nombre de sus representantes de ventas.
SELECT c.nombre_cliente,e.nombre,e.apellido1,ofi.ciudad as ciudad FROM pago p INNER JOIN cliente c ON p.codigo_cliente=c.codigo_cliente INNER JOIN empleado e ON c.codigo_empleado_rep_ventas=e.codigo_empleado INNER JOIN oficina ofi ON e.codigo_oficina=ofi.codigo_oficina; -- 3. Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
...
SELECT c.nombre_cliente,e.nombre,e.apellido1,e.apellido2,ofi.ciudad AS ciudad FROM cliente c INNER JOIN empleado e ON c.codigo_empleado_rep_ventas=e.codigo_empleado INNER JOIN oficina ofi ON e.codigo_oficina = ofi.codigo_oficina WHERE c.ciudad='Fuenlabrada'; -- 5. Lista la dirección de las oficinas que tengan clientes en Fuenlabrada.
SELECT c.nombre_cliente,e.nombre,e.apellido1, e.apellido2,ofi.ciudad AS ciudad_oficina FROM cliente c INNER JOIN empleado e  ON c.codigo_empleado_rep_ventas= e.codigo_empleado INNER JOIN oficina ofi ON e.codigo_oficina = ofi.codigo_oficina; -- 5. Devuelve el nombre de los clientes y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
SELECT nombre_cliente, gama FROM cliente NATURAL JOIN gama_producto; -- 6. Devuelve un listado con el nombre de los empleados junto con el nombre de sus jefes.
SELECT nombre_cliente, fecha_esperada, fecha_entrega FROM cliente c INNER JOIN pedido p ON c.codigo_cliente=p.codigo_cliente WHERE fecha_esperada > fecha_entrega;-- 7. Devuelve un listado que muestre el nombre de cada empleados, el nombre de su jefe y el nombre del jefe de sus jefe.
SELECT  e.nombre AS nombre_empleado, j.nombre AS nombre_jefe  FROM empleado j INNER JOIN empleado e ON j.codigo_empleado=e.codigo_jefe; -- 8. Devuelve el nombre de los clientes a los que no se les ha entregado a tiempo un pedido.
SELECT  e.nombre AS nombre_empleado, j.nombre AS nombre_jefe, j2.nombre AS nombre_jefe2 FROM empleado j INNER JOIN empleado e ON j.codigo_empleado=e.codigo_jefe INNER JOIN empleado j2 ON j.codigo_jefe=j2.codigo_empleado ; -- 9. Devuelve un listado de las diferentes gamas de producto que ha comprado cada cliente.